...
import { Shirt, Clock, Info, Loader2 } from "lucide-react";
...
const Seragam = () => {
  ...
  const fetchUniforms = async () => {
    // Asumsi nama tabelnya 'uniforms'
    const { data, error } = await supabase
      .from("uniforms")
      .select("*")
      .order("id", { ascending: true }); // Urutkan sesuai ID atau hari
  ...
  useLayoutEffect(() => {
    // Jangan jalankan animasi kalau masih loading atau data kosong
    if (loading || uniformData.length === 0) return;

    let ctx = gsap.context(() => {
      // Refresh ScrollTrigger biar posisi akurat setelah gambar dimuat
    ...
    return () => ctx.revert();
  }, [loading, uniformData]); // Dependency penting! Biar animasi jalan pas data masuk
    ...
    >
      ...
      <div className="bg-white py-16 lg:py-20 border-b border-gray-100">
        <div className="container mx-auto px-4 lg:px-8 text-center">
          <span className="header-pill inline-block py-1 px-3 rounded-full bg-orange-100 text-orange-600 text-sm font-bold mb-4 invisible">
            Tata Tertib
          </span>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              <Loader2
                ...
                size={40}
              />
              <p className="text-gray-500">Memuat data seragam...</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {uniformData.map((item) => (
                <div
                  key={item.id}
                  className="uniform-card group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:border-orange-500/30 transition-all duration-300 flex flex-col h-full hover:-translate-y-2 invisible"
                >
                  {/* IMAGE CONTAINER */}
                  <div className="relative h-80 overflow-hidden bg-gray-200">
                    <div className="absolute top-4 left-4 z-10">
                      <span className="flex items-center gap-2 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm text-orange-600 font-bold text-sm">
                        <Clock size={16} /> {item.day}
                      </span>
                    </div>

                    <img
                      src={item.image_url} // Pastikan kolom di DB namanya image_url
                      alt={item.title}
                  ...
                  <div className="p-6 flex flex-col flex-grow relative">
                    <div className="absolute top-0 left-0 w-0 h-1 bg-orange-500 group-hover:w-full transition-all duration-500 ease-out"></div>
                    ...
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <Shirt
                        size={20}
                        className="text-gray-300 group-hover:text-orange-600 transition-colors"
                      />
                    </div>
          ...
          {!loading && (
            <div className="note-card mt-16 bg-orange-50 border border-orange-100 rounded-xl p-6 lg:p-8 flex flex-col md:flex-row items-center gap-6 text-center md:text-left shadow-sm invisible">
              <div className="info-icon-pulse p-4 bg-white rounded-full shadow-md text-orange-600">
                <Info size={32} />
              </div>
              <div>
