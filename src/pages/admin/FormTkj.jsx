...
const FormTkj = () => {
  ...
  // Handle Tambah Topik ke Array
  const addTopic = (e) => {
    e.preventDefault();
    if (!currentTopic.trim()) return;
    setTopics([...topics, currentTopic]);
    setCurrentTopic(""); // Reset input
  };

  // Handle Hapus Topik dari Array
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      topics: topics, // Supabase akan otomatis convert ke JSONB
    };
    ...
    try {
      ...
      navigate("/admin/tkj"); // Kembali ke dashboard
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 lg:p-12 font-sans flex items-center justify-center">
      <div className="w-full max-w-2xl">
        ...
        <Link
          to="/admin/tkj"
          className="inline-flex items-center text-gray-500 hover:text-teal-600 mb-6 transition font-medium"
        >
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          <div className="bg-teal-600 p-6 text-white">
            <h1 className="text-2xl font-bold">
              {isEditMode ? "Edit Materi TKJ" : "Tambah Materi Baru"}
            </h1>
            <p className="text-teal-100 text-sm mt-1">
              {isEditMode
                ? "Perbarui informasi kurikulum yang ada."
                : "Masukan data kurikulum pembelajaran baru."}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-teal-500 outline-none appearance-none bg-white cursor-pointer"
                  >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-teal-500 outline-none transition"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mb-4">
                <input
                  ...
                  placeholder="Ketik topik lalu tekan Enter atau tombol +"
                  className="flex-1 px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-teal-500 outline-none transition"
                />
                <button
                  type="button"
                  onClick={addTopic}
                  className="bg-teal-100 text-teal-700 px-4 py-2 rounded-lg hover:bg-teal-200 transition active:scale-95"
                >
                  <Plus size={24} />
                </button>
              </div>
            ...
            <div className="pt-4">
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-teal-600 text-white font-bold text-lg py-4 rounded-xl hover:bg-teal-700 hover:shadow-lg hover:shadow-teal-500/30 transition-all transform active:scale-[0.99] flex justify-center items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
