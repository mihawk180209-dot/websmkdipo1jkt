...
import {
  ...
  Layers,
  AlertCircle,
  Palette, // Saya ganti ikon BookOpen jadi Palette agar lebih 'DKV', opsional
} from "lucide-react";
...
const ManageDkv = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Pastikan tabel di supabase bernama 'dkv_curriculum'
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Yakin ingin menghapus materi DKV ini?")) return;
  ...
  // (Tetap beda warna tiap kelas agar mudah dibedakan, tapi XII pakai Pink/Rose biar senada)
  const getClassBadge = (level) => {
    const styles = {
      X: "bg-indigo-100 text-indigo-800 border-indigo-200",
      XI: "bg-fuchsia-100 text-fuchsia-800 border-fuchsia-200",
      XII: "bg-rose-100 text-rose-800 border-rose-200",
    };
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
              {/* Menggunakan warna Purple */}
              <Palette className="text-purple-600" />
              Manajemen Kurikulum DKV
            </h1>
            <p className="text-gray-500 mt-1">
              Kelola data materi pembelajaran Desain Komunikasi Visual.
            </p>
          </div>
          <Link
            to="/admin/dkv/tambah" // Route DKV
            className="group flex items-center gap-2 bg-purple-600 text-white px-6 py-3 rounded-xl font-semibold shadow-lg hover:bg-purple-700 hover:shadow-purple-500/30 transition-all transform hover:-translate-y-1"
          >
            <Plus
              size={20}
                ...
                >
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold border ${getClassBadge(
                          item.class_level,
                        )}`}
                      >
                    ...
                    <div className="mb-6 min-h-[60px]">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {topics.slice(0, 3).map((t, i) => (
                          <span
                            key={i}
                            // Badge topik warna purple muda
                            className="text-xs bg-purple-50 text-purple-700 px-2 py-1 rounded border border-purple-100"
                          >
                            {t}
                          </span>
                        ))}
                    ...
                    <div className="flex gap-3 mt-auto pt-4 border-t border-gray-100">
                      <Link
                        to={`/admin/dkv/edit/${item.id}`} // Route Edit DKV
